    ...
    <>
      ...
      <header
        className={`
          ...
          sm:top-4 sm:left-6 sm:right-6
          md:left-8 md:right-8
          lg:left-12 lg:right-12
          z-50 rounded-xl
          transition-all duration-500
          ${scrollY > 20
            ? 'bg-[#22063d]/95 backdrop-blur-xl shadow-2xl shadow-black/20 py-2 sm:py-3 border border-[#a548ff]/10'
      ...
      >
        <nav className="max-w-7xl mx-auto px-3 sm:px-5 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            >
              <div className="relative">
                {/* Fixed: was w-24 h-24 on mobile — now always 40px, 48px on sm+ */}
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:rotate-6">
                  <img src={Logo} alt="Areej Abumuhfouz" className="w-full h-full object-contain" />
                </div>
                <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-[#cbd5e1] to-[#a548ff] opacity-0 group-hover:opacity-50 blur-xl transition-all duration-300" />
              </div>
            </a>
            ...
            {/* ── Desktop CTA & Social ── */}
            <div className="hidden lg:flex items-center gap-3 xl:gap-4">
              {/* Social icons — only on xl+ to avoid cramping */}
              <div className="hidden xl:flex items-center gap-2">
                {socialLinks.map(({ Icon, href, label }) => (
                  <a
                    key={label}
              ...
              <a
                href={CV}
                download
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl border-2 border-white/20 text-white font-semibold hover:bg-white/5 hover:border-white/40 transition-all duration-300 hover:scale-105"
              >
                <Download className="w-4 h-4" />
                <span>CV</span>
              </a>
            ...
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="lg:hidden w-10 h-10 rounded-xl bg-white/5 backdrop-blur-xl border border-white/10 flex items-center justify-center hover:bg-white/10 hover:border-[#cbd5e1]/50 transition-all duration-300 relative z-50"
              aria-label="Toggle menu"
            >
              {mobileMenuOpen
      ...
      >
        ...
        {/* Menu content */}
        <div className="relative h-full flex flex-col justify-center items-center px-6 py-24 overflow-y-auto">
          {/* Decorative orbs */}
          <div className="absolute top-20 left-10 w-56 h-56 bg-[#cbd5e1]/10 rounded-full blur-3xl animate-pulse pointer-events-none" />
          <div className="absolute bottom-20 right-10 w-72 h-72 bg-[#a548ff]/15 rounded-full blur-3xl animate-pulse pointer-events-none" style={{ animationDelay: '1s' }} />

          {/* Nav links */}
          <nav className="relative z-10 flex flex-col items-center gap-1 w-full max-w-sm mb-10">
            {navItems.map((item, index) => (
              <a
                key={item.name}
                href={item.href}
                onClick={() => setMobileMenuOpen(false)}
                className="group w-full text-center py-4 text-3xl sm:text-4xl font-bold text-white/80 hover:text-white transition-all duration-300 relative"
                style={{ animation: `slideIn 0.45s ease-out ${index * 0.08 + 0.05}s both` }}
              >
                <span className="relative z-10">{item.name}</span>
                <span className="absolute inset-0 bg-gradient-to-r from-[#cbd5e1]/8 to-[#a548ff]/8 rounded-2xl opacity-0 group-hover:opacity-100 transition-all duration-300 scale-95 group-hover:scale-100" />
              </a>
            ))}
          </nav>
          ...
          <div
            ...
            style={{ animation: 'slideIn 0.45s ease-out 0.42s both' }}
          >
            {/* CV download — same as desktop */}
            <a
              href={CV}
              download
              onClick={() => setMobileMenuOpen(false)}
          ...
          <div
            ...
            style={{ animation: 'slideIn 0.45s ease-out 0.5s both' }}
          >
            {socialLinks.map(({ Icon, href, label }) => (
              <a
                key={label}
                ...
                rel="noopener noreferrer"
                onClick={() => setMobileMenuOpen(false)}
                className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 hover:border-[#cbd5e1]/50 hover:scale-110 transition-all duration-300 group"
              >
                <Icon className="w-5 h-5 text-white/70 group-hover:text-[#cbd5e1] transition-colors" />
              </a>
            ))}
          </div>
          ...
          <a
            href="mailto:areejabumahfouz@gmail.com"
            onClick={() => setMobileMenuOpen(false)}
            className="relative z-10 mt-8 text-[#a548ff]/70 hover:text-[#cbd5e1] transition-colors text-sm"
            style={{ animation: 'slideIn 0.45s ease-out 0.58s both' }}
          >
            areejabumahfouz@gmail.com
          </a>
        </div>
